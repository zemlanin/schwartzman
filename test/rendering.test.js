...
describe("rendering", function() {
  ...
  describe("section", function() {
    var tmpl = eval(
      schwartzman(`<div>{{# user }}[{{ name }}]{{/ user }}({{ name }})</div>`)
    );
    ...
    it("if", function() {
      assert.equal(
        ...
        ReactDOMServer.renderToStaticMarkup(
          React.createElement(tmpl, { user: true, name: "Anatoly Dyatlov" })
        )
      );
      ...
      assert.equal(
        ...
        ReactDOMServer.renderToStaticMarkup(
          React.createElement(tmpl, { user: false, name: "Anatoly Dyatlov" })
        )
      );
      ...
      assert.equal(
        "<div>()</div>",
        ReactDOMServer.renderToStaticMarkup(
          React.createElement(tmpl, { user: false, name: null })
        )
      );
      ...
      assert.equal(
        "<div>(Anatoly Dyatlov)</div>",
        ReactDOMServer.renderToStaticMarkup(
          React.createElement(tmpl, { name: "Anatoly Dyatlov" })
        )
      );
    });
    ...
    it("scope", function() {
      ...
      assert.equal(
        ...
        ReactDOMServer.renderToStaticMarkup(
          React.createElement(tmpl, { user: {}, name: "Ulana Khomyuk" })
        )
      );
  ...
  it("comments", function() {
    ...
    assert.equal(
      `<div><i></i><i></i></div>`,
      ReactDOMServer.renderToStaticMarkup(React.createElement(tmpl, {}))
    );
  });
  ...
  it("valueless attributes", function() {
    var tmpl = eval(
      schwartzman("<span {{# y }}hidden{{/ y }}>hidden should be falsey</span>")
    );
    var tmpl2 = eval(
      schwartzman("<span {{# y }}hidden{{/ y }}>hidden should be truthy</span>")
    );
  ...
  it("naked attribute values", function() {
    var tmpl = eval(schwartzman("<span id=a>id'd</span>"));
  ...
  it("computed style", function() {
    var tmpl = eval(
      schwartzman("<span style={{sm}}>should have small font-size</span>")
    );
  ...
  it("escaping curlybraces", function() {
    ...
    assert.equal(
      "<span>Jason</span>",
      ReactDOMServer.renderToStaticMarkup(
        React.createElement(eval(schwartzman("<span>{{name}}</span>")), {
          name: "Jason"
        })
      )
    );
